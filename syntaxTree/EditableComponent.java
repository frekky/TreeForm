 ...
 * <br>
 * <br>
 * A subclass of JComponent with editable Graphics2D text.
 * <br>
 * This type of class became necessary to accomplish easy printing and picture
 * exporting.  This class contains important listeners that communicate hit test
 * and key listeners.  Also, there are commands for detecting and relaying text width.
 */
public class EditableComponent extends JComponent {
	 ...
	 * @uml.associationEnd 
	 * @uml.property name="mHead" multiplicity="(0 1)" qualifier="TRANSFORM:java.awt.font.TextAttribute
 ...
 * <br>
 * <br>
 * This is a Key listener, it only works when the implementing class has focus.
 */
 ...
 * of the highlight to the insertion index, set the Head Observers, and repaint the
 * screen so the new carat position shows up.
 * <br>
 * <br>
 * RIGHT: add one to the insertionIndex, set the beginning and end 
 * of the highlight to the insertion index, set the Head Observers, and repaint the
 * screen so the new carat position shows up.
 * <br>
 * <br>
 * DELETE: delete the highlighted text (if any), then step the highlight forward one,
 * and delete this new highlightted text.
 * <br>
 * <br>
 * BACKSPACE: delete the highlighted text (if any), then step the highlight back one,
 * and delete this new highlighted text.
 */
...
/**
 * if the key is NOT a CTRL masked key, and not a delete or backspace key,
 * insert the text, with the current observed text attributes, into the
 * visible textstring.  Then redisplay the tree.
 */
 ...
 * to the hittest location, turn on the carat, and repaint
 */
...
/**
 * Set the layout origin for the carat
 * This is calculated by getting the size of the object, and the position of the
 * center of the textlayout within this object.
 */
 ...
 * If the SHIFT key is also pressed, a movement flag is set, and
 * the move cursor is displayed.
 * <br>
 * If the RIGHT MOUSE BUTTON is pressed, a relevant popup menu is displayed.
 * <br>
 * Otherwise, a pointest is conducted, and the insertion index, highlight begin,
 * and highlight end are activated.  Components are then repainted.
 * <br>
 * <br>
 */
...
/**
 * 
 * @param pME the mouse event you wish to test
 * @return the current insertion index (which letter)
 * <br>
 * <br>
 * First you check the mouse click position relative to the center of the textlayout
 * <br>
 * Then you adjust the clickX and clickY position in relation to the scale of the object
 * <br>
 * Then you test the hit location in an unscaled environment, and return the correct
 * index.
 */
		private int pointTest(MouseEvent pME) {
			...
			if (clickY < (lRectangle.getHeight() / 2)) {
				clickY =
					(float) ((lRectangle.getHeight() / 2)
						- (((lRectangle.getHeight() / 2) - clickY)
							/ (Sizer.scaleHeight()
								* getUserInternalFrame().getScale())));
			} else {
				clickY =
					(float) ((lRectangle.getHeight() / 2)
						+ ((clickY - (lRectangle.getHeight() / 2))
							/ (Sizer.scaleHeight()
								* getUserInternalFrame().getScale())));
			}

			TextHitInfo currentHit =
				mTextLayoutHead.hitTestChar(clickX, clickY);
			return currentHit.getInsertionIndex();
		}
/**
 * if the move flag is on, releasing the mouse is the completion of a move
 * or association event
 * <br>
 * In this case, it is important to test if the mouse was released over 
 * a potention parent node.
 * <br>
 * If yes, then test to see if a structure was moved.
 * <br>
 * If yes, call the moveSyntacticStructure method
 * <br>
 * If no, call the associateSyntacticStructure method.
 * <br>
 * If the move flag was NOT on, then just set the final highlight, and repaint.
 * <br>
 */
...
/**
 * test to see if the SHIFT key is pressed.  if it is, test to see
 * what subclass of EditableComponent was called.
 * <br>
 * If it is a SyntacticStructure, translate the Structure (move all of it's
 * children, features, and associations with it).
 * <br>
 * If it is a SyntacticFeature, translate the feature only.
 * <br>
 * Otherwise the mouseDragged event is an attempt to highlight text,
 ...
 */
		public void mouseDragged(MouseEvent pME) {
			if ((pME.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) != 0)
			{
				...
				{
					mUserInternalFrame.getSyntaxFacade().translateSyntacticFeature(
...
/**
 * Constructor
 * @param pUserInternalFrame The UserInternalFrame that will hold this
 * EditableComponent
 * <br>
 * This constructor sets default heads, and adds the mouse, mousemotion, and key
 * listeners.
 */
	public EditableComponent(UserInternalFrame pUserInternalFrame) {
		...
		Font lFont = new Font("Doulos SIL", Font.BOLD, getUserInternalFrame().getProperties().fontSize());
 ...
 * same as the graphics object for this Component)
 * <br>
 * <br>
 * This component makes a point based on the text width and height,
 * <br>
 * Then it makes a scaled point based on the screen scale.
 * <br>
 * Then it passes the scaling to the graphics object to the component
 * scales the vector drawing correctly.
 * <br>
 * The color and anti-aliasing settings are then turned on
 * <br>
 * Then the textLayout is generated and rendered in the graphics object
 * <br>
 * If this box is highlighted, a light blue highlight is drawn filling the
 * entire box
 * <br>
 * If the carat is turned on, a black line is drawn appropriately within the text
 * <br>
 * Lastly, if text is highlighted, the colors for that text are inverted
 * using the XOR painting call
 * <br>
 */
	public void paint(Graphics pG) {
		...
		Float lPoint2D =
			new Point2D.Float(this.getTextWidth(), this.getTextHeight());

		Float lPoint2DScaled =
			new Point2D.Float(
				(int) (lPoint2D.getX()
					* Sizer.scaleWidth()
		...
		lGraphics2D.scale(
			...
			Sizer.scaleHeight() * this.getUserInternalFrame().getScale());
		...
		if (this.getCarat()) {
			lGraphics2D.translate(getZero(this.getTextWidth() - mTextLayoutHead.getBounds().getWidth())/2,
				mTextLayoutHead.getAscent());
			Shape[] carets =
				mTextLayoutHead.getCaretShapes(getInsertionIndex());
...
/**
 * 
 * @param pWidth Sets the text width as discovered in the testXY method.
 */
...
/**
 * 
 * @param pTextHeight Sets the text height as discovered in the testXY method.
 */
...
/**
 * 
 * @param pUserInternalFrame Sets the InternalFrame for this component - for convenience
 * 
 */
...
/**
 * 
 * @param pHead All insertion of heads (text) involves recalibrating the size and 
 * setting the headlength value.
 */
...
/**
 * 
 * @param pHeadLength Sets the number of characters in the text
 */
...
/**
 * 
 * @param pB sets whether the box is highlighted blue or not.
 */
...
/**
 * 
 * @param pCarat Sets the visibility of the Carat.
 */
...
/**
 * 
 * @param pWidth Sets the real text width, which may be different from the
...
/**
 * 
 * @param pInsertionIndex Sets the insertion index, no less than 0, nor more
 * than the maximum character length.
 */
...
/**
 * 
 * @return Returns the insertion index, or location of the Carat in the text.
 */
...
/**
 * 
 * @param pHighlightBegin Sets the point at which text highlighting begins.
 */
...
/**
 * 
 * @param pHighlightEnd Sets the point at which text highlighting ends.
 */
 ...
 */
	public AttributedString getClip() {
		return new AttributedString(
			mHead.getIterator(null, mHighlightBegin, mHighlightEnd));
	}
/**
 * 
 * @param pInsert The new piece of Attributed Text to be inserted into the head.
 * @param pIndex The beginning of the insertion point.
 * <br>
 * <br>
 * This formula cycles through the original head and the newly inserted piece,
 * pulling out the UNICODE text.
 * <br>
 * Then a new string of text is built with the old string to the insertion point,
 * the new string, and the rest of the old string.
 * <br>
 * Then the attributedString of the old head is stepped through, one step at a time,
 * the attributes are copied out and pasted into the new AttributedString.
 * <br>
 * This process inserts the old Map information into the correct index locations
 * of the new AttributedString, one character at a time.
 * <br>
 * The result is a new AttributedString with all font and format information preserved.
 */
	public void insertHead(AttributedString pInsert, int pIndex) {
		...
		for (char c = lInsertIterator.first();
			c != CharacterIterator.DONE;
			c = lInsertIterator.next()) {
			lInsertString += c;
		}
		String lFinalString =
			lString.substring(0, pIndex)
				+ lInsertString
...
/**
 * This Operation deletes highlighted text from the head (AttributedString text).
 * <br>
 * <br>
 * The old AttributedString is iterated, and a new text string is generated with only the correct
 * Text.
 * <br>
 * Then the old  AttributedString is iterated again, and Map information containing
 * the font and transformations is pasted into the new string, one character at a time.
 * <br>
 * Finally, the new AttributedString is stored using setHead to replace the old one.
 *
 */
		...
		if (getHighlightBegin() != getHighlightEnd()) {
			...
			for (char c = lIterator.first();
				...
				if (lI < getHighlightBegin() || lI >= getHighlightEnd()) {
					...
					lAttributedString.addAttributes(
						lIterator.getAttributes(),
						lJ,
						lJ + 1);
					lJ++;
...
/**
 * 
 * @param map sets the attributes of highlighted texts with the passed in attribute
 * set.
 */
	public void setHighlight(Map map) {
		if (getHighlightBegin() != getHighlightEnd()) {
			getHead().addAttributes(
				map,
				getHighlightBegin(),
				getHighlightEnd());
			testXY();
...
/**
 * Tests the textWidth and textHeight and returns the correct values to
 * the EditableComponent Object.
 * <br>
 * <br>
 * TextHeight is based on the ascent + descent + 1 pixel,
 * TextWidth is based on the Advance + 4 pixels. (I would like a better
 * formula, but this will have to do for now.)
 *
 */
	public void testXY() {
		int lWidth = 0;
		...
		FontRenderContext frc =
			((Graphics2D) getUserInternalFrame().getGraphics())
		...
		try {
			...
			Font lFont = new Font("Doulos SIL", Font.PLAIN, getUserInternalFrame().getProperties().fontSize());
		...
		lWidth = lWidth < getUserInternalFrame().getProperties().minTextWidth() ? getUserInternalFrame().getProperties().minTextWidth() : lWidth;
 ...
 * is vital to make the GUI store accurate information about highlighted text.
 *
 */
	public void setHeadObservers()
	{
		...
		if (lFont != null) {
			mUserInternalFrame.getObservableFont().setValue(
				lFont.getName());
			mUserInternalFrame.getObservableFontSize().setValue(
				lFont.getSize());
			mUserInternalFrame.getObservableFontBold().setValue(
				lFont.isBold());
			...
			else if(((TransformAttribute)lFont.getAttributes().get(TextAttribute.TRANSFORM)).getTransform().getTranslateY() == -3)
			{
				...
				mUserInternalFrame.getObservableFontSuperscript().setValue(true);
			}
			else
			{
				mUserInternalFrame.getObservableFontSubscript().setValue(false);
